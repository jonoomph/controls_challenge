...
def process_file(model_name, model_path, file_index, window_size, tinyphysicsmodel):
    try:
        ...
        for _ in range(tinyphysics.CONTEXT_LENGTH, len(sim.data)):
            sim.step()
...
def start_testing(filter=None, logging=True, window_size=22, training_files=60, executor=ThreadPoolExecutor()):
    global MAX_TRAINING_ROWS

    model_costs = defaultdict(lambda: {"total_cost": 0, "file_count": 0})
    tasks = []

    for model_name in tqdm(sorted(os.listdir("onnx")), disable=not logging or filter):
    ...
    for future in tqdm(as_completed(tasks), total=len(tasks), disable=not logging, position=1):
